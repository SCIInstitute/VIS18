function probability = yPositiveRInfiniteFlipped(k, a, b, c, d, p, critical, start, finish)
...
if (start > 0)
    ...
    elseif(c4 < c2)
        ...
        elseif (c4 <= 1) && (1 < c2)
            % Change in revised code
            % commented previous
            %probability = (log(d + k) - log((k^2 + p)/(b + k)))/((a - b)*(c - d)) - (log(c + k) - log((k^2 + p)/(a + k)))/((a - b)*(c - d));
            % p4InfiniteB
